...
import json
import os
from exo.inference.tinygrad.models.llama import Transformer, TransformerShard, convert_from_huggingface, fix_bf16, sample_logits, SampleState
...
log_file_path = Path("inference.log")  # Укажите путь к файлу логов
log_file_path.parent.mkdir(parents=True, exist_ok=True)  # Создайте директорию для файла логов, если она не существует

logging.basicConfig(
    level=logging.DEBUG,
    filename=str(log_file_path),
    filemode='a',  # 'a' для добавления в файл, 'w' для перезаписи файла
...
def build_transformer(model_path: Path, shard: Shard, model_size="8B", device=None):
    ...
    logging.debug(f"Weights loaded and model built: {model}")  # Логирование построенной модели
    return model
...
class TinygradDynamicShardInferenceEngine(InferenceEngine):
    def __init__(self, shard_downloader: ShardDownloader):
        ...
        self.sample_state = SampleState(logits_shape=(MODEL_PARAMS["8B"]["args"]["vocab_size"],))  # Инициализация sample_state
    ...
    async def sample(self, x: np.ndarray, temp=TEMPERATURE, top_p: float = TOP_P) -> np.ndarray:
        ...
        def sample_wrapper():
            return sample_logits(
                ...
                ALPHA_F,
                ALPHA_P,
                self.sample_state  # Передача sample_state
    ...
    async def encode(self, shard: Shard, prompt: str) -> np.ndarray:
        ...
        logging.debug(f"Encoded tokens: {tokens}")  # Логирование закодированных токенов
        return await asyncio.get_running_loop().run_in_executor(self.executor, np.array, tokens)

    async def decode(self, shard: Shard, tokens) -> str:
        await self.ensure_shard(shard)
        decoded_text = await asyncio.get_running_loop().run_in_executor(self.executor, self.tokenizer.decode, tokens)
        logging.debug(f"Decoded text: {decoded_text}")  # Логирование декодированного текста
        return decoded_text
    ...
    async def ensure_shard(self, shard: Shard):
        ...
        if self.shard != shard:
            ...
            logging.debug(f"Tokenizer loaded from: {tokenizer_path}")  # Логирование пути к токенизатору
            self.shard = shard
