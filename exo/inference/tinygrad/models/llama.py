...
def precompute_freqs_cis(dim: int, end: int, theta: float = 10000.0, dtype=dtypes.float32, rope_scaling: Optional[Dict[str, float]] = None) -> Tensor:
...
def apply_rotary_emb(xq: Tensor, xk: Tensor, freqs_cis: Tensor) -> Tuple[Tensor, Tensor]:
    assert freqs_cis.shape[1] == xq.shape[1] == xk.shape[1], f"freqs_cis shape mismatch {freqs_cis.shape} xq:{xq.shape} xk:{xq.shape}"
...
class Attention:
    ...
    def __call__(self, x: Tensor, start_pos: Union[Variable, int], freqs_cis: Tensor, mask: Optional[Tensor], cache: Optional[Tensor] = None) -> Tensor:
        ...
        if cache is not None:
            # Ensure cache has the same dtype as xk and xv
            cache_dtype = cache.dtype
            xk = xk.cast(cache_dtype)
            xv = xv.cast(cache_dtype)
            # Update the cache
...
class TransformerBlock:
    def __init__(self, dim: int, hidden_dim: int, n_heads: int, n_kv_heads: int, norm_eps: float, max_context: int, linear=nn.Linear, feed_forward=FeedForward):
        ...
        self.attention_norm = nn.RMSNorm(dim, norm_eps)  # Removed dtype argument
        self.ffn_norm = nn.RMSNorm(dim, norm_eps)  # Removed dtype argument
...
class SampleState:
    def __init__(self, logits_shape):
        self.alpha_counter = Tensor.zeros_like(Tensor.empty(logits_shape, dtype=dtypes.int32).contiguous())

def sample_logits(logits: Tensor, temp: float, k: int, p: float, af: float, ap: float, sample_state: SampleState):
    assert logits.ndim == 1, "only works on 1d tensors"
    assert 0 <= p <= 1, "p must be between 0 and 1"
    assert 0 <= k <= logits.numel(), "k must be between 0 and numel"
    if temp < 1e-6: return logits.argmax().reshape(1)
    if af or ap:
        if not hasattr(sample_state, "alpha_counter"):
            sample_state.alpha_counter = Tensor.zeros_like(logits, dtype=dtypes.int32).contiguous()
        logits = logits - (sample_state.alpha_counter * af + (sample_state.alpha_counter > 0) * ap)
    ...
    if k:
        ...
        output_cumsum = output[::-1].cumsum()[::-1] + t.sum()  # Замена _cumsum на cumsum
    ...
    if af or ap:
        sample_state.alpha_counter = (counter == output_token).where(sample_state.alpha_counter + 1, sample_state.alpha_counter)

    logger.debug(f"Sampled token: {output_token.item()}")  # Логирование сгенерированного токена
    return output_token
...
class Transformer:
    def __init__(
        ...
        self.norm = nn.RMSNorm(dim, norm_eps)  # Removed dtype argument
        self.tok_embeddings = nn.Embedding(vocab_size, dim)  # Removed dtype argument
        self.output = nn.Linear(dim, vocab_size, bias=False)  # Removed dtype argument
        if tie_word_embeddings:
            self.output.weight = self.tok_embeddings.weight
        self.max_context = max_context
        self.freqs_cis = precompute_freqs_cis(
            dim // n_heads, self.max_context * 2, rope_theta, dtype=dtypes.float32, rope_scaling=rope_scaling
        ).contiguous()
    ...
    def forward_base(self, x: Tensor, start_pos: Union[Variable, int], cache: Optional[List[Tensor]] = None):
        ...
        if self.shard.is_last_layer():
            ...
            logger.debug(f"Logits shape: {logits.shape}, Logits: {logits}")  # Логирование логитов
            return logits
        else:
            return h
...
def convert_from_huggingface(weights: Dict[str, Tensor], model: Transformer, n_heads: int, n_kv_heads: int):
    ...
    logger.debug(f"Converted weights: {sd.keys()}")  # Логирование ключей преобразованных весов
    return sd
